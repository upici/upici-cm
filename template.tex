...
% 12pt: taille des fontes.
% twoside: miseen page pour recto-verso (par exemple numéro de page alternés
% droite/gauche, etc)

\usepackage[utf8]{inputenc} % Pour taper les caractères accentués -- ceci
                            % signifie que le fichier doit être encodé
                            % (enregistré sur l'ordinateur avec
                            % l'encodage utf-8)
\usepackage[T1]{fontenc}
% On peut changer la police par défaut, en utilisant un paquet additionel
\usepackage{lmodern}
\usepackage[a4paper]{geometry}
\geometry{vmargin=2cm,hmargin=2.5cm,nohead}

\usepackage{mathtools} % charge amsmath avec des corrections de bugs
\usepackage{amsfonts} % fontes pour les ensembles N, Z, Q, R...
\usepackage{amssymb} % nombreux symboles
\usepackage{amsthm} % styles de théorèmes
...
\DeclareGraphicsExtensions{.png} % Mettre ici la liste des extensions des
                                 % fichiers images que l'on souhaite utiliser

\usepackage[french]{babel} % Pour le français, babel est le dernier
                           % chargé, sauf exception
% Exception venant après babel:
% eurosym
% varioref
% caption
% subcaption
...
\newcommand{\rr}{{\mathbb R}} % L'ensemble R
\newcommand{\cc}{{\mathbb C}} % L'ensemble C
\newcommand{\nn}{{\mathbb N}} % L'ensemble N
\newcommand{\zz}{{\mathbb Z}} % L'ensemble Z

% Les informations de la page de titre (page de titre séparée pour un 'report').
\title{Mon premier rapport en \LaTeX}
\author{
  Moi Même\thanks{
    \href{mailto:yves.coudiere@u-bordeaux.fr}{\tt yves.coudiere@u-bordeaux.fr}}
  \and
